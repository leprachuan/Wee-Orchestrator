...
class OpenCodeAgenticTest:
    ...
    def run_opencode_test(self, name, prompt, timeout=30):
        ...
        try:
            ...
            self.results["tests_failed"] += 1
            self.results["failures"].append({"test": name, "error": "Timeout"})
            return False, "TIMEOUT"
        except Exception as e:
            self.results["tests_failed"] += 1
            self.results["failures"].append({"test": name, "error": str(e)})
            return False, str(e)
...
At the end, list all 5 items you remembered."""
        ...
        ctx_pass, ctx_out = self.run_opencode_test(
            "context_persistence", ctx_prompt, timeout=45
        )
...
Ensure step 3 uses info from step 1."""
        ...
        flow_pass, flow_out = self.run_opencode_test(
            "agentic_flow", flow_prompt, timeout=45
        )
        ...
        edge_prompts = [
            (
                "long_ctx",
                "Discuss LLM context windows extensively. " * 20
