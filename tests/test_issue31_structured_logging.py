...
class TestIssue31StructuredLogging:
    ...
    def test_issue_31_json_format_produces_valid_json(self):
        ...
        stream = StringIO()
        with patch.dict(
            os.environ, {"LOG_FORMAT": "json", "LOG_LEVEL": "DEBUG"}, clear=False
        ):
            am = _import_am()
            am._configure_logging()
        ...
        for h in root.handlers:
            if isinstance(h.formatter, am._JsonFormatter):
                ...
                break
        assert (
            json_handler is not None
        ), "_JsonFormatter handler not found after LOG_FORMAT=json"
        ...
        assert (
            "timestamp" in record
            or "time" in record
            or "asctime" in record
            or "ts" in record
        ), "JSON log record missing timestamp field"
        assert (
            "level" in record or "levelname" in record
        ), "JSON log record missing level field"
        assert (
            "message" in record or "msg" in record
    ...
    def test_issue_31_no_diagnostic_print_to_stderr(self):
        ...
        matches = re.findall(
            r"\bprint\s*\(.*?file\s*=\s*sys\.stderr", source, re.DOTALL
        )
    ...
    def test_issue_31_json_formatter_class_exists(self):
        ...
        assert hasattr(
            am, "_JsonFormatter"
        ), "agent_manager missing '_JsonFormatter' class"
        assert issubclass(
            am._JsonFormatter, logging.Formatter
