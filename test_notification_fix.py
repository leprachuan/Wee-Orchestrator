...
def test_notification_preference_storage():
    ...
    session_data = session_mgr.get_or_create_session_data(n8n_session_id, identity=identity)
    ...
    assert loaded_data.get("notification_preference") == "off", "Notification preference not stored"
...
def test_notification_preference_inheritance():
    ...
    webui_data = session_mgr.get_or_create_session_data(webui_session_id, identity=identity)
    ...
    for n8n_sid, data in session_map.items():
        ...
        if data.get("identity") == identity:
            ...
            print(f"Found session for {identity}: {n8n_sid} (channel={data.get('channel')})")
    
    assert len(matching_sessions) >= 2, f"Expected at least 2 sessions, got {len(matching_sessions)}"
    ...
    assert defaults.get("notification_preference") == "off", f"Expected 'off' but got {defaults.get('notification_preference')}"
...
def test_multi_channel_inheritance():
    ...
    print(f"Found {len(matching_sessions_same_channel)} sessions on same channel (webui)")
    print(f"Found {len(matching_sessions_other_channel)} sessions on other channels")
    
    # Collect preferences
    for n8n_sid, data in matching_sessions_same_channel + matching_sessions_other_channel:
        pref = data.get("notification_preference")
        if pref:
            defaults["notification_preference"] = pref
