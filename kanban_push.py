...
def push_board(session_id, state):
    ...
    components = [
        ...
        {'type': 'text', 'text': f'Runtime: {runtime} · Model: {model} · Timeout: {timeout}', 'muted': True},
        {'type': 'divider'},
        {'type': 'row', 'children': [
            {'type': 'metric', 'id': 'status-metric',  'label': 'Status',  'value': overall_status},
            {'type': 'metric', 'id': 'runtime-metric', 'label': 'Runtime', 'value': runtime},
            {'type': 'metric', 'id': 'model-metric',   'label': 'Model',   'value': model},
            {'type': 'metric', 'id': 'timeout-metric', 'label': 'Timeout', 'value': timeout},
        ]},
        {'type': 'divider'},
        ...
        {'type': 'board', 'columns': [
            ...
            },
            {
                ...
                'title': '🔄 In Progress',
                'items': [
                    {'id': p['id'], 'title': p['title'], 'status': p.get('status', 'running')}
            ...
            },
            {
                'id': 'col-done',
                'title': '✅ Done',
                'items': [{'id': p['id'], 'title': p['title'], 'status': 'done'} for p in done_items],
            },
        ]},
    ]
    ...
    if log_lines:
        components += [
            {'type': 'divider'},
            {'type': 'log', 'id': 'build-log', 'label': '📋 Build Log', 'lines': log_lines[-30:]},
        ]

    c.render(components)


def cmd_init(session_id, phases_json, title='Background Task', runtime='copilot',
             model='', timeout='60 min', first_phase_running=True):
...
def cmd_update(session_id, phase_id, status, log_line=''):
    ...
    if not state:
        print(f'[kanban_push] ERROR: no state found for session {session_id}. Run init first.')
        sys.exit(1)
    ...
    save_state(session_id, state)
    push_board(session_id, state)
    print(f'[kanban_push] {phase_id} → {status}' + (f' | {log_line}' if log_line else ''))
