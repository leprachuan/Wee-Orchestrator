...
def test_notifications_command():
    ...
    session_info = am.initialize_session(
        n8n_session_id, test_prompt, "webex", "test-user"
    )
    ...
    result = am.process_command(
        n8n_session_id, "/notifications current", "webex", "test-user"
    )
    ...
    result = am.process_command(
        n8n_session_id, "/notifications on", "webex", "test-user"
    )
    ...
    print("\n--- Test 3: Disable Notifications (off) ---")
    result = am.process_command(
        n8n_session_id, "/notifications off", "webex", "test-user"
    )
    ...
    result = am.process_command(
        n8n_session_id, "/notifications mute", "webex", "test-user"
    )
    ...
    print("\n--- Test 5: Invalid Command ---")
    result = am.process_command(
        n8n_session_id, "/notifications invalid", "webex", "test-user"
    )
