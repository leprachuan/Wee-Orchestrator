...
# Stall timeout: if in-progress with no running task for this long, re-dispatch
...
def minutes_since(iso_time: str) -> float:
    """Return minutes elapsed since iso_time."""
    if not iso_time:
        return None
    try:
...
def get_open_wee_dev_issues() -> list[dict]:
    """Fetch open issues with wee-dev label."""
    cmd = [
        "gh", "issue", "list",
        "--repo", REPO,
        "--label", "wee-dev",
...
def is_task_running(task_id: str) -> bool:
    ...
    try:
        ...
        return False  # API unreachable → assume not running
...
def passes_safety_gate(item: dict) -> bool:
    """Check if issue is approved for dispatch."""
    author = item.get("author", {}).get("login", "")
    if author == OWNER_LOGIN:
        return True
    # Non-owner issue needs approval
    labels = [l["name"] for l in item.get("labels", [])]
    if "wee-dev:needs-approval" not in labels:
        ...
        add_comment(
            ...
            f"Filed by @{author}; only @{OWNER_LOGIN} issues are auto-dispatched.",
        )
    return False
...
def build_wee_dev_prompt(item: dict) -> str:
    """Build prompt for wee-dev to work on issue."""
    return (
        ...
        "2. Implement the fix/feature on the dev host (192.168.1.100):\n"
        "   SSH: ssh root@192.168.1.100\n"
        "   Path: /opt/n8n-copilot-shim-dev/\n\n"
        "3. Work directly on the dev branch (no feature branches):\n"
...
def dispatch_wee_dev(item: dict, state: dict) -> None:
    """Dispatch wee-dev to work on an issue."""
...
def run_pipeline() -> None:
    ...
    if in_progress:
        ...
        # Task not running — check stall timeout
        ...
        if mins is not None and mins < STALL_TIMEOUT_MINUTES:
            log(
                f"wee-dev task for #{item['number']} ended within {mins:.1f}min "
                f"(< {STALL_TIMEOUT_MINUTES}min stall timeout) — may still be processing"
            )
            return
    ...
    # No in-progress — pick next queued issue
    if queued:
